...
An object in thermodynamic equilibrium with its environment emits electromagnetic 
radiation; the spectrum of this radiation is defined by the temperature of the object.
...
1918  Max Planck awarded the Nobel Prize in Physics for his discovery of the energy quanta
1921  Albert Einstein awarded the Nobel Prize in Physics for the Photoelectric Effect
...
collisions. The Ideal Gas Law instead predicts the average behavior of the entire volume 
...
the X,Y,Z direction; rotation about the X,Y,Z axis; and longitudinal oscillations of the
atoms that make up the molecules. According to the Equipartition theorem each of these degrees 
...
Maxwell's equations show that light waves propagate due to an oscillation between electric and 
magnetic fields. The speed of light propagation in vacuum is related to the Vacuum Permittivity constants.
...
the time. Planck was later awarded the Nobel Prize in Physics for his discovery of the energy
...
In 1900 The Rayleigh-Jeans Law made use of the Equipartition theorem to predict the Black Body
spectrum. Their approach starts by assuming that light waves are in thermodynamic equilibrium with
a chamber that contains the waves. Rayleigh and Jeans calculated the number of modes of light
waves that could be contained in a chamber, vs the frequency of the light waves. According to the
Equipartition Theorem, each mode would have Kb * T energy. The Rayleigh-Jeans Law for black body
Spectral Radiance says that the Spectral Radiance at a frequency is Kb * T * the number of
light wave modes at that frequency. This equation yields results that are very close to 
experiment and the Planck Law for low frequencies. However, the equation is way off at high frequencies, and
...
In 1905 Albert Einstein worked on the photoelectric effect, which led him to conceive of the
quanta of light. Einstein was later awarded the Nobel Prize in Physics for this. The quanta
...
Planck had earlier thought it was the oscillator energy that was quantized.
...
```
    ...
    calc_rj() function returns the correct value.
```

The Spectral Radiance calculated using the Rayleigh-Jeans Law agrees with experiment and the Planck Law
at low frequencies; however, at high frequencies Rayleigh-Jeans Law diverges dramatically from Planck. 
...
The Spectral Radiance for a given wavelength is computed as follows:
...
- their is a small difference between the plots of Mine vs Planck
