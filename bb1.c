...
double maxwell_boltzmann_probability_2(double energy);
...
double calc_mine(double f)
{
    ...
    avg_energy_quantized = sum_energy_quantized / MAX / 3.0;
...
void calc_mine_init(void)
{
    ...
    max_e = 35*KT;
    delta_e = max_e / 10000; //xxx

    array = calloc(10000+10, sizeof(double));

    //array[max_array++] = 0;
    for (e = 0; e < max_e; e += delta_e) {
        p = maxwell_boltzmann_probability_2(e);
        sum_p += p;
...
double maxwell_boltzmann_probability(double velocity)
{
    ...
    if (k == 0) {
        k = 8 * M_PI * sqrt(MASS) / pow(2 * M_PI * KT, 1.5);
    }

    probability = k * energy * exp(-energy / KT);
